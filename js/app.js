/** Data model
 * Includes data points for the map.
 */
var Model = {
    ...
    markers: [{
        ...
        title: 'AZ Museum of Natural History',
        lat: 33.416651,
        lng: -111.833587,
        url: 'http://www.azmnh.org',
        highlight: ko.observable(false)
    }, {
        title: "Children's Museum of Phoenix",
        lat: 33.450398,
        lng: -112.064641,
        url: 'http://www.childrensmuseumofphoenix.org',
        highlight: ko.observable(false)
    }, {
        title: "Luci's Healthy Marketplace",
        lat: 33.523987,
        lng: -112.047926,
        url: 'http://www.lucishealthymarketplace.com/',
        highlight: ko.observable(false)
    }, {
        title: 'Phoenix Zoo',
        lat: 33.451236,
        lng: -111.949955,
        url: 'http://www.phoenixzoo.org',
        highlight: ko.observable(false)
    }, {
        title: 'Phoenix Art Museum',
        lat: 33.467302,
        lng: -112.073412,
        url: 'http://www.phxart.org/',
        highlight: ko.observable(false)
    }, {
        title: 'Great Arizona Puppet Theatre',
        lat: 33.461050,
        lng: -112.078073,
        url: 'http://www.azpuppets.org',
        highlight: ko.observable(false)
    }, {
        title: 'Musical Instrument Museum',
        lat: 33.667457,
        lng: -111.978671,
        url: 'http://www.mim.org',
        highlight: ko.observable(false)
    }, {
        title: 'McCormick-Stillman Railroad Park',
        lat: 33.538154,
        lng: -111.924569,
        url: 'http://www.therailroadpark.com',
        highlight: ko.observable(false)
    }, {
        title: 'Heard Museum',
        lat: 33.472474,
        lng: -112.072201,
        url: 'http://www.heard.org',
        highlight: ko.observable(false)
    }]
};
var ViewModel = function() {
    ...
    var initMap = function() {
        ...
        if (typeof window.google === 'object' && typeof window.google.maps ===
            ...
            for (var x = 0; x < markerList.length; x++) {
                var markPos = new google.maps.LatLng(markerList[x].lat,
                    markerList[x].lng);
    ...
    self.filteredArray = ko.computed(function() {
        return ko.utils.arrayFilter(self.markerArray(), function(
            marker) {
            return marker.title.toLowerCase().indexOf(self.query()
                .toLowerCase()) !== -1;
        });
    }, self);
    ...
    self.filteredArray.subscribe(function() {
        var diffArray = ko.utils.compareArrays(self.markerArray(),
            self.filteredArray());
    ...
    self.getPictures = function() {
        ...
        if (marker !== null) {
            ...
            var searchUrl =
                ...
                '&api_key=13406652d1a1ae4996e57aa9b96cc051&text=' +
                textSearch +
                '&license=1%2C2%2C3%2C4%2C5%2C6%2C7&content_type=1&lat=' +
                marker.position.lat() + '&lon=' + marker.position.lng() +
                '&radius=1&radius_units=km&per_page=15&page=1' +
            ...
            $.getJSON(searchUrl).done(function(data) {
                ...
                alert(
                    "Unable to get photos from Flickr at this time."
                );
            });
        } else {
    ...
    self.prevPhoto = function() {
        ...
        if (i !== 0) {
            ...
            self.lightboxUrl(self.currentPhotos()[self.currentPhotos().length -
                1]);
        }
    };
    ...
    function parseSearchResults(data) {
            ko.utils.arrayForEach(data.photos.photo, function(photo) {
                var photoLink = 'https://farm' + photo.farm +
